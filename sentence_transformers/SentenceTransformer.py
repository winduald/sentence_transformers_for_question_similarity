...
class SentenceTransformer(nn.Sequential):
    def __init__(self, model_name_or_path: str = None, modules: Iterable[nn.Module] = None, device: str = None, max_seq_length: int = None):
        ...
        if model_name_or_path is not None and model_name_or_path != "":
            ...
            if model_path is not None:
                ...
                for module_config in contained_modules:
                    ...
                    print(module_class)
                    if module_class is BERT or module_class is RoBERTa:
                        module = module_class.load(os.path.join(model_path, module_config['path']), max_seq_length=max_seq_length)
                    else:
        ...
        for epoch in trange(epochs, desc="Epoch"):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05):
                ...
                if evaluation_steps > 0 and training_steps % evaluation_steps == 0:
                    ...
                    # self._eval_during_training(evaluator, None, save_best_model, epoch, training_steps)
                    for loss_model in loss_models:
                        loss_model.zero_grad()
                        loss_model.train()

            self._eval_during_training(evaluator, output_path, save_best_model, epoch, -1)
            # self._eval_during_training(evaluator, None, save_best_model, epoch, -1)
    ...
    def evaluate(self, evaluator: SentenceEvaluator, output_path: str = None):
        ...
        #     raise Exception("output file not specificed")
        return evaluator(self, output_path)

    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps):
        """Runs evaluation during the training"""
        if evaluator is not None:
            score = evaluator(self, output_path=None, epoch=epoch, steps=steps)
